<h1 align="center">hey, i’m tushaar 👋</h1>
<p align="center">writer → marketer → builder — making things i wish existed</p>

<p align="center">
  <img src="https://komarev.com/ghpvc/?username=tushaarmehtaa&label=profile+views" alt="profile views" />
</p>

---

### 🛠 current projects

- **smart-fill** — a chrome extension that knows *you*, scrapes pages, and fills boring forms in seconds  
- **trend-hopper** — create viral tweets based on what’s picking up + what’s worked in the past  
- **scriptwriter** — the fastest way to turn raw ideas into ready-to-record video scripts  
- **tweetwriter** — write tweets in your voice, based on past bangers + scattered thoughts  

---

### 🧰 stack i love messing with

![Next.js](https://img.shields.io/badge/-Next.js-000?logo=nextdotjs&logoColor=white)
![Supabase](https://img.shields.io/badge/-Supabase-3ECF8E?logo=supabase&logoColor=white)
![OpenAI](https://img.shields.io/badge/-OpenAI-412991?logo=openai&logoColor=white)
![Anthropic](https://img.shields.io/badge/-Anthropic-000?logoColor=white)
![TailwindCSS](https://img.shields.io/badge/-Tailwind-38B2AC?logo=tailwindcss&logoColor=white)
![LangChain](https://img.shields.io/badge/-LangChain-000?logo=langchain&logoColor=white)
![Vercel](https://img.shields.io/badge/-Vercel-000?logo=vercel&logoColor=white)

---

### 🎙️ a bit about me

`23 yrs old` · `self-taught everything`  
`fascinated by talent, systems, incentives & the internet as a playground`  

i ship because the alternative is regret.

---

### 💬 say hi

- 🧵 [@tusharmehta](https://twitter.com/tushaarmehtaa)  
- 🪄 [newsletter](https://tushaarmehtaa.substack.com/)  

---

<p align="center">
  <i>“no depth, no fun.”</i>
</p>
